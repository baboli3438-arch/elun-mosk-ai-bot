...
# =====================================================================
# 1. GÜVENLİK, SAYFA VE API AYARLARI
# =====================================================================
...
if not groq_api_key:
    st.error("🚨 API Anahtarı Bulunamadı! Lütfen secrets veya .env ayarlarını kontrol edin.")
    st.stop()
...
def handle_chat_request(user_message):
    # Çift tetiklemeyi önlemek için kontrol
    if st.session_state.chat_history[-1]["content"] == user_message:
        return
    ...
    try:
        ...
        # HTML içinde kırılmaları önlemek için satır atlamalarını düzeltelim
...
# =====================================================================
# 3. URL PARAMETRELERİNİ YAKALAMA (IFRAME KÖPRÜSÜ)
# =====================================================================
query_params = st.query_params

if "msg" in query_params:
    user_query = query_params["msg"]
    # Parametreyi hemen temizleyelim ki sayfa yenilendiğinde sonsuz döngüye girmesin
...
# =====================================================================
# 4. SİBERPUNK ARAYÜZ (HTML / TAILWIND SHAPE)
# =====================================================================
...
<head>
  ...
  <style>
    ...
    .scanline::after {{
      ...
      animation: scan 4.5s linear infinite; opacity: 0.25; pointer-events: none;
    }}
...
<body class="min-h-screen flex">
    ...
    <div class="flex items-center gap-4 mb-10">
      ...
      <div>
        ...
        <p class="text-red-400 flex items-center gap-2 text-sm">
          <span class="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span> ONLINE • MEME MODE
        </p>
      </div>
    </div>
  ...
  <div class="flex-1 flex flex-col scanline">
    ...
    <div class="p-6 border-t border-red-600/30 glass">
      <div class="max-w-4xl mx-auto relative">
        ...
        <button onclick="sendMessage()" class="absolute right-4 top-1/2 -translate-y-1/2 bg-gradient-to-r from-red-500 to-orange-500 w-14 h-14 rounded-2xl flex items-center justify-center hover:scale-110 transition">
  ...
  <script>
    ...
    function sendMessage() {{
      ...
      // iframe'in asılı kalmasını önlemek için window.parent yönlendirmesi optimize edildi
      setTimeout(() => {{
...
# =====================================================================
# 5. STREAMLIT FRAME RENDER (DÜZ METİN HATASINI ÖNLEYEN KISIM)
# =====================================================================
st.markdown(
    """
    <style>
        #MainMenu, footer, header {visibility: hidden;}
        .stApp {margin: 0px; padding: 0px; background-color: black;}
...
# HTML şablonunu güvenli iframe bileşeni olarak render ediyoruz
